...
async def original(request):
    ...
    if strip_regex is not None:
        ...
        if original_uri != image_uri:
            log.debug(
                "strip_regex transformed image_uri %s -> %s", original_uri, image_uri
            )
    ...
    if origin:
        ...
        if not is_safe_path(image_uri):
            log.warning(
                "tenant %r: requested unsafe path traversal %r", tenant, image_uri
            )
        ...
        if original_qp:
            ...
            log.debug(
                "preserving query params for fixed-origin request: %s", request_url
            )
    else:
